        ...
        <div className="space-y-2">
          {BRANDS.map((brand) => (
            <label key={brand} className="flex items-center gap-2 cursor-pointer group">
              <input type="checkbox" checked={selectedBrands.includes(brand)} onChange={() => toggleBrand(brand)} className="w-4 h-4 rounded border-[#E0E0E0] accent-[#1565C0]" />
      ...
      <div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <input type="text" placeholder="Đến" value={priceRange[1].toLocaleString("vi-VN")} className="flex-1 border border-[#E0E0E0] rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:border-[#1565C0]" readOnly />
          </div>
        </div>
      </div>
      <div className="border-t border-[#E0E0E0]" />
        ...
        <div className="space-y-2">
          {[5, 4, 3, 2, 1].map((r) => (
            <label key={r} className="flex items-center gap-2 cursor-pointer group">
              <input type="checkbox" checked={selectedRatings.includes(r)} onChange={() => toggleRating(r)} className="w-4 h-4 rounded accent-[#1565C0]" />
        ...
        <div className="flex-1 min-w-0">
          <div className="bg-white rounded-xl border border-[#E0E0E0] p-3 mb-4 flex items-center gap-3 flex-wrap">
            ...
            <div className="flex border border-[#E0E0E0] rounded-lg overflow-hidden">
              <button onClick={() => setViewMode("grid")} className={`p-1.5 ${viewMode === "grid" ? "bg-[#1565C0] text-white" : "text-[#757575] hover:bg-gray-50"}`}><LayoutGrid size={16} /></button>
              <button onClick={() => setViewMode("list")} className={`p-1.5 ${viewMode === "list" ? "bg-[#1565C0] text-white" : "text-[#757575] hover:bg-gray-50"}`}><List size={16} /></button>
            </div>
          </div>
                  ...
                  <div key={product.id} className="bg-white rounded-xl border border-[#E0E0E0] p-4 flex gap-4 hover:shadow-md transition-shadow cursor-pointer" onClick={() => router.push(`/products/${product.id}`)}>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-3 mt-2">
                        ...
                        {product.comparePrice && <span className="text-sm text-[#757575] line-through">{product.comparePrice.toLocaleString("vi-VN")}đ</span>}
                      </div>
                    </div>
                  </div>
                )
