...
export default function AdminOrdersPage() {
  ...
  const columns = [
    { key: "id", header: "Mã đơn", render: (order: any) => <span className="font-medium text-[#1565C0] hover:underline cursor-pointer">{order.id}</span> },
    {
    ...
    { key: "paymentMethod", header: "Thanh toán", render: (order: any) => <span className="text-xs bg-gray-100 text-[#757575] px-2 py-0.5 rounded">{order.paymentMethod}</span> },
    { key: "status", header: "Vận chuyển", render: (order: any) => <OrderStatusBadge status={order.status} /> },
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      />
      <TableDataTable data={filteredOrders} columns={columns} selectedIds={selectedOrders} onToggleSelect={toggleSelect} onToggleAll={toggleAll} idKey="id" emptyMessage="Không tìm thấy đơn hàng" />
      <div className="px-5 py-3 border-t border-[#E0E0E0] flex items-center justify-between text-sm text-[#757575]">
        ...
        <div className="flex gap-1">
          <button onClick={() => setPagination(prev => ({ ...prev, page: Math.max(1, prev.page - 1) }))} disabled={pagination.page === 1} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-gray-100 disabled:opacity-50">←</button>
          {Array.from({ length: Math.min(5, pagination.totalPages) }, (_, i) => i + 1).map((n) => (
            <button key={n} onClick={() => setPagination(prev => ({ ...prev, page: n }))} className={`w-8 h-8 flex items-center justify-center rounded-lg ${n === pagination.page ? "bg-[#1565C0] text-white" : "hover:bg-gray-100"}`}>{n}</button>
          ))}
          <button onClick={() => setPagination(prev => ({ ...prev, page: Math.min(pagination.totalPages, prev.page + 1) }))} disabled={pagination.page === pagination.totalPages} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-gray-100 disabled:opacity-50">→</button>
        </div>
      </div>
    </div>
  );
