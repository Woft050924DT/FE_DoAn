...
import {
  ...
  Lock, LogOut, ChevronRight, Plus, Edit2, Trash2
} from "lucide-react";
          ...
          {activeSection === "overview" && (
            <div className="space-y-5">
              ...
              <div className="bg-gradient-to-r from-[#1565C0] to-[#1976D2] rounded-2xl p-6 text-white">
                <h2 className="text-xl font-bold mb-1">Xin chào, {USER.name.split(" ").pop()}! 👋</h2>
                <p className="text-white/70 text-sm">Thành viên từ {USER.memberSince}</p>
              </div>
                  ...
                  <div key={addr.id} className={`bg-white border-2 rounded-2xl p-5 ${addr.isDefault ? "border-[#1565C0]" : "border-[#E0E0E0]"}`}>
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-2">
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${addr.type === "home" ? "bg-green-100 text-[#2E7D32]" : "bg-blue-100 text-[#1565C0]"}`}>
                    ...
                    <div className="flex gap-2 mt-4">
                      ...
                      {!addr.isDefault && (
                        <>
                          <button className="flex items-center gap-1 text-xs text-[#E53935] border border-[#E53935]/30 px-3 py-1.5 rounded-lg hover:bg-red-50">
                            <Trash2 size={11} /> Xóa
                          </button>
                          <button className="text-xs text-[#757575] border border-[#E0E0E0] px-3 py-1.5 rounded-lg hover:bg-gray-50">
                            Đặt mặc định
                          </button>
                        </>
                      )}
