...
const formatAddress = (addr: Address) =>
  [addr.address_line1, addr.address_line2, addr.ward, addr.district, addr.city]
    .filter(Boolean)
    .join(", ");
...
const NOTIFICATIONS = [
  {
    ...
    message: "Đơn hàng #DH2024003 đã được giao đến bưu cục.",
    time: "2 giờ trước",
    unread: true,
  },
  {
    ...
    message: "Flash Sale cuối tuần – Giảm đến 50% hàng ngàn sản phẩm!",
    time: "5 giờ trước",
    unread: true,
  },
  {
    ...
    message: "Hãy đánh giá sản phẩm bạn đã mua để nhận điểm thưởng!",
    time: "1 ngày trước",
    unread: false,
  },
  {
    ...
    message: "Đơn hàng #DH2024001 đã được thanh toán thành công.",
    time: "2 ngày trước",
    unread: false,
  },
];
...
const FALLBACK_ADDRESSES = [
  ...
  },
  {
    ...
    name: "Nguyễn Văn An",
    phone: "0901234567",
    address: "456 Đinh Tiên Hoàng, Phường Đa Kao",
    city: "Q1, TP. Hồ Chí Minh",
    isDefault: false,
  },
];
...
export function ScreensAccount() {
  ...
  const setDefaultAddress = async (addressId: string) => {
    try {
      const updated = await profileService.updateAddress(addressId, {
        is_default: true,
      });
      setAddresses((prev) =>
        prev.map((addr) => ({
          ...addr,
          is_default: addr.address_id === updated.address_id,
        })),
      );
    } catch (err) {
      console.error(err);
    ...
    <div className="max-w-6xl mx-auto px-4 py-6">
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1 min-w-0">
            ...
            <div className="space-y-5">
              ...
              <div className="bg-gradient-to-r from-[#1565C0] to-[#1976D2] rounded-2xl p-6 text-white">
                <h2 className="text-xl font-bold mb-1">
                  Xin chào, {displayUser.name.split(" ").pop()}! 👋
                </h2>
                <p className="text-white/70 text-sm">
                  Thành viên từ {displayUser.memberSince}
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  ...
                  {
                    label: "Đã chi tiêu",
                    value: formatCurrency(USER.totalSpent).replace("₫", "đ"),
                    icon: "💰",
                    color: "text-[#E53935]",
                    small: true,
                  },
                  ...
                  <div
                    key={stat.label}
                    className="bg-white rounded-xl border border-[#E0E0E0] p-4 text-center"
                  >
                    <div className="text-2xl mb-1">{stat.icon}</div>
                    <p
                      className={`font-bold ${stat.small ? "text-sm" : "text-xl"} ${stat.color}`}
                    >
                      {stat.value}
                    </p>
                    <p className="text-xs text-[#757575] mt-0.5">
                    ...
                    <thead className="bg-[#F5F6FA]">
                      <tr>
                        {[
                          ...
                          <th
                            key={h}
                            className="text-left px-4 py-2.5 text-xs font-semibold text-[#757575]"
                          >
                            {h}
                          </th>
                        ))}
                    ...
                    <tbody className="divide-y divide-[#F5F6FA]">
                      {recentOrders.map((order) => (
                        <tr key={order.id} className="hover:bg-[#F5F6FA]">
                          ...
                          <td className="px-4 py-3">
                            <button className="text-xs text-[#1565C0] hover:underline">
                              Xem
                            </button>
                          </td>
                        </tr>
              ...
              <div className="bg-white rounded-2xl border border-[#E0E0E0] p-5">
                ...
                <div className="space-y-3">
                  {NOTIFICATIONS.slice(0, 3).map((notif) => (
                    <div
                      key={notif.id}
                      className={`flex items-start gap-3 p-3 rounded-xl transition-colors ${notif.unread ? "bg-blue-50/50" : ""}`}
                    >
                      <span className="text-xl shrink-0">{notif.icon}</span>
                      <div className="flex-1">
                        <div className="flex items-start justify-between gap-2">
                          <p className="font-medium text-sm text-[#212121]">
                            {notif.title}
                          </p>
                          {notif.unread && (
                            <div className="w-2 h-2 bg-[#E53935] rounded-full mt-1 shrink-0" />
                          )}
                        </div>
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex items-center justify-between mt-3">
                        ...
                        <div className="flex gap-2">
                          <button className="text-xs border border-[#E0E0E0] px-3 py-1.5 rounded-lg hover:bg-gray-50">
                            Xem chi tiết
                          </button>
                          {order.status === "delivered" && (
                            <button className="text-xs bg-amber-50 text-amber-700 border border-amber-200 px-3 py-1.5 rounded-lg hover:bg-amber-100">
                              Đánh giá
                            </button>
                          )}
                          {order.status === "pending" && (
                            <button className="text-xs bg-red-50 text-[#E53935] border border-red-200 px-3 py-1.5 rounded-lg hover:bg-red-100">
                              Hủy đơn
                            </button>
                          )}
                          <button className="text-xs bg-[#E53935] text-white px-3 py-1.5 rounded-lg hover:bg-[#C62828]">
                            Mua lại
                          </button>
                        </div>
                      </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {accountAddresses.map((addr) => (
                  <div
                    key={addr.id}
                    className={`bg-white border-2 rounded-2xl p-5 ${addr.isDefault ? "border-[#1565C0]" : "border-[#E0E0E0]"}`}
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-2">
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full font-medium ${addr.type === "home" ? "bg-green-100 text-[#2E7D32]" : "bg-blue-100 text-[#1565C0]"}`}
                        >
                    ...
                    <div className="flex gap-2 mt-4">
                      <button className="flex items-center gap-1 text-xs text-[#1565C0] border border-[#1565C0]/30 px-3 py-1.5 rounded-lg hover:bg-blue-50">
                        <Edit2 size={11} /> Sửa
                      </button>
                      {!addr.isDefault && (
                        <>
                          <button
                            onClick={() =>
                              "apiAddressId" in addr &&
                              deleteAddress(addr.apiAddressId)
                            }
                          ...
                          <button
                            onClick={() =>
                              "apiAddressId" in addr &&
                              setDefaultAddress(addr.apiAddressId)
                            }
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        {notif.unread && (
                          <div className="w-2 h-2 bg-[#E53935] rounded-full mt-1.5 shrink-0" />
                        )}
                      </div>
          ...
          {!["overview", "orders", "addresses", "notifications"].includes(
            activeSection,
          ) && (
