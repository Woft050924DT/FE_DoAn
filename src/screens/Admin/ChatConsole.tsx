...
import {
  Search, Send, Smile, Paperclip, Phone, Video, MoreVertical,
  Tag, ChevronDown, X, AlertCircle, Bot, User, UserCheck
} from "lucide-react";
...
const CHAT_CONVERSATIONS = [
  {
    id: "1",
    customer: { name: "Nguyễn Văn An", email: "an.nguyen@gmail.com", avatar: "NA", phone: "0901234567" },
    status: "open",
    priority: "urgent",
    lastMessage: "Tôi muốn hỏi về chính sách đổi trả...",
    time: "2 phút",
    unread: 3,
    assignedStaff: "Minh Tuấn",
    intent: "Đổi trả hàng",
    sentiment: 0.35,
  },
  {
    id: "2",
    customer: { name: "Trần Thị Bình", email: "binh.tran@gmail.com", avatar: "TB", phone: "0912345678" },
    status: "waiting",
    priority: "high",
    lastMessage: "Đơn hàng của tôi chưa nhận được",
    time: "15 phút",
    unread: 1,
  ...
  {
    id: "3",
    customer: { name: "Lê Minh Châu", email: "chau.le@gmail.com", avatar: "LC", phone: "0923456789" },
    status: "bot",
    priority: "normal",
    lastMessage: "Cho tôi xem các sản phẩm iPhone",
    time: "32 phút",
    unread: 0,
    assignedStaff: "Bot AI",
...
const QUICK_REPLIES_LIST = [
  ...
  "Bạn có thể truy cập vào mục đơn hàng để kiểm tra.",
];
...
export function ScreensAdminChatConsole() {
  ...
  return (
    <div className="flex h-full bg-[#F5F6FA]" style={{ height: "calc(100vh - 64px)" }}>
        ...
        <div className="p-4 border-b border-[#E0E0E0]">
          <div className="relative mb-3">
            ...
            <input
              ...
              className="w-full pl-8 pr-4 py-2 border border-[#E0E0E0] rounded-lg text-sm focus:outline-none focus:border-[#1565C0] bg-[#F5F6FA]"
            />
          </div>
            ...
            {FILTERS.map((f) => (
              <button
                ...
                className={`px-2.5 py-1 text-xs rounded-full whitespace-nowrap transition-colors ${
                  activeFilter === f ? "bg-[#1565C0] text-white" : "text-[#757575] hover:bg-gray-100"
                }`}
              >
                {f}
              </button>
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-[#F5F6FA]">
          {CHAT_CONVERSATIONS.map((conv) => (
            <button
              ...
              className={`w-full p-3.5 text-left hover:bg-[#F5F6FA] transition-colors ${
                activeConv === conv.id ? "bg-blue-50/60 border-r-2 border-[#1565C0]" : ""
              }`}
            >
              <div className="flex items-start gap-2.5">
                <div className="relative shrink-0">
                  <div className="w-9 h-9 bg-[#E53935] rounded-full flex items-center justify-center text-white text-sm font-bold">
                    {conv.customer.avatar}
                  </div>
                  <div className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-white ${
                    conv.priority === "urgent" ? "bg-[#E53935]" : conv.priority === "high" ? "bg-amber-400" : "bg-green-400"
                  }`} />
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between mt-1">
                    ...
                    {conv.unread > 0 && (
                      <span className="w-4 h-4 bg-[#E53935] rounded-full text-white text-[9px] flex items-center justify-center font-bold">
                        {conv.unread}
                      </span>
                    )}
                  </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-white border-r border-[#E0E0E0]">
        ...
        <div className="px-4 py-3 border-b border-[#E0E0E0] flex items-center gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <select className="text-xs border border-[#E0E0E0] rounded-lg px-2 py-1.5 focus:outline-none bg-white">
              ...
              <option>Bot AI</option>
              <option>Lan Phương</option>
            </select>
            <select className="text-xs border border-[#E0E0E0] rounded-lg px-2 py-1.5 focus:outline-none bg-white">
                  ...
                  <div className="max-w-[70%]">
                    <div className={`px-3 py-2 rounded-2xl text-sm ${
                      msg.type === "user"
                        ...
                        : msg.type === "staff"
                        ? "bg-[#2E7D32] text-white rounded-tl-sm"
                        : "bg-white text-[#212121] rounded-tl-sm shadow-sm border border-[#E0E0E0]"
                    }`}>
                      {msg.text}
                    </div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {QUICK_REPLIES_LIST.map((qr) => (
                <button
                  ...
                  className="text-xs bg-[#F5F6FA] hover:bg-[#E0E0E0] px-2.5 py-1 rounded-full text-[#212121] transition-colors"
                >
                  {qr.slice(0, 30)}{qr.length > 30 ? "..." : ""}
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="p-3 border-t border-[#E0E0E0] bg-white">
          <div className="flex items-center gap-2 bg-[#F5F6FA] rounded-xl px-3 py-2">
            ...
            <button
              onClick={sendMessage}
              className="w-8 h-8 bg-[#1565C0] rounded-full flex items-center justify-center hover:bg-[#0D47A1] transition-colors"
            >
      ...
      <div className="w-72 shrink-0 bg-white overflow-y-auto hidden xl:block">
        ...
        {/* Recent Orders */}
        <div className="p-4 border-b border-[#E0E0E0]">
          <p className="text-xs font-semibold text-[#212121] mb-3">Đơn hàng gần đây</p>
          <div className="space-y-2">
            {ORDERS.length > 0 ? ORDERS.slice(0, 3).map((order: any) => (
              <div key={order.id} className="flex items-center justify-between text-xs">
                <span className="text-[#1565C0] font-medium">{order.id}</span>
                <OrderStatusBadge status={order.status} size="sm" />
                <span className="text-[#757575]">{formatCurrency(order.total)}</span>
              </div>
            )) : (
              <p className="text-xs text-[#757575]">Chưa có đơn hàng</p>
            )}
          </div>
        </div>
              ...
              <button
                ...
                className={`text-xs px-2.5 py-1 rounded-full border transition-colors ${
                  activeTags.includes(tag)
                    ...
                    : "border-[#E0E0E0] text-[#757575] hover:border-[#1565C0]"
                }`}
              >
                {tag}
              </button>
            ))}
            <button className="text-xs px-2 py-1 rounded-full border border-dashed border-[#E0E0E0] text-[#757575] hover:border-[#1565C0]">
              + Thêm
            </button>
          </div>
        </div>
        ...
        <div className="p-4 border-b border-[#E0E0E0]">
          ...
          <div className="space-y-3">
            ...
            <div>
              <p className="text-xs text-[#757575] mb-2">Cảm xúc</p>
              <div className="flex items-center justify-between text-xs mb-1">
                <span className="text-[#2E7D32]">Tích cực</span>
                <span className="font-semibold">{Math.round((currentConv?.sentiment || 0) * 100)}%</span>
              </div>
              <div className="w-full bg-gray-100 rounded-full h-2">
                <div
                  className={`h-2 rounded-full ${
                    (currentConv?.sentiment || 0) > 0.6 ? "bg-[#2E7D32]" :
                    (currentConv?.sentiment || 0) > 0.3 ? "bg-amber-400" : "bg-[#E53935]"
                  }`}
                  style={{ width: `${(currentConv?.sentiment || 0) * 100}%` }}
                />
              </div>
            ...
            <div>
              <p className="text-xs text-[#757575] mb-1">Độ tin cậy</p>
              <div className="flex items-center gap-2">
                <div className="flex-1 bg-gray-100 rounded-full h-2">
                  <div className="h-2 bg-[#1565C0] rounded-full" style={{ width: "87%" }} />
                </div>
                <span className="text-xs font-semibold text-[#1565C0]">87%</span>
              </div>
            </div>
          </div>
        </div>
