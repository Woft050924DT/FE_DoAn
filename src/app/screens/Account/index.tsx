                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
                  <div className="bg-white rounded-2xl border border-[#E0E0E0] shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
                    <div className="flex items-center justify-between px-5 py-4 border-b border-[#E0E0E0]">
                      ...
                      <button type="button" onClick={closeAddressModal} className="p-1 rounded-lg hover:bg-gray-100 text-[#757575]" aria-label="Đóng">
                        <X size={18} />
                      </button>
                    </div>
                    <div className="p-5 space-y-3">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) => updateAddressForm({ address_type: e.target.value as "home" | "office" })}
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                          ...
                          <input value={addressForm.full_name} onChange={(e) => updateAddressForm({ full_name: e.target.value })} className="w-full border border-[#E0E0E0] rounded-lg px-3 py-2 text-sm" placeholder="Nguyễn Văn A" />
                        </div>
                        <div>
                          <label className="text-xs font-medium text-[#757575] block mb-1">Số điện thoại *</label>
                          <input value={addressForm.phone} onChange={(e) => updateAddressForm({ phone: e.target.value })} className="w-full border border-[#E0E0E0] rounded-lg px-3 py-2 text-sm" placeholder="0901234567" />
                        </div>
                      </div>
                      <div>
                        <label className="text-xs font-medium text-[#757575] block mb-1">Địa chỉ (số nhà, đường) *</label>
                        <input value={addressForm.address_line1} onChange={(e) => updateAddressForm({ address_line1: e.target.value })} className="w-full border border-[#E0E0E0] rounded-lg px-3 py-2 text-sm" placeholder="123 Đường ABC" />
                      </div>
                      <div>
                        <label className="text-xs font-medium text-[#757575] block mb-1">Địa chỉ dòng 2</label>
                        <input value={addressForm.address_line2} onChange={(e) => updateAddressForm({ address_line2: e.target.value })} className="w-full border border-[#E0E0E0] rounded-lg px-3 py-2 text-sm" placeholder="Tòa nhà, căn hộ (tuỳ chọn)" />
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <div>
                          <label className="text-xs font-medium text-[#757575] block mb-1">Phường/Xã</label>
                          <input value={addressForm.ward} onChange={(e) => updateAddressForm({ ward: e.target.value })} className="w-full border border-[#E0E0E0] rounded-lg px-3 py-2 text-sm" />
                        </div>
                        <div>
                          <label className="text-xs font-medium text-[#757575] block mb-1">Quận/Huyện</label>
                          <input value={addressForm.district} onChange={(e) => updateAddressForm({ district: e.target.value })} className="w-full border border-[#E0E0E0] rounded-lg px-3 py-2 text-sm" />
                        </div>
                        <div>
                          <label className="text-xs font-medium text-[#757575] block mb-1">Tỉnh/TP *</label>
                          <input value={addressForm.city} onChange={(e) => updateAddressForm({ city: e.target.value })} className="w-full border border-[#E0E0E0] rounded-lg px-3 py-2 text-sm" placeholder="Hà Nội" />
                        </div>
                      </div>
                      <div>
                        <label className="text-xs font-medium text-[#757575] block mb-1">Mã bưu điện</label>
                        <input value={addressForm.postal_code} onChange={(e) => updateAddressForm({ postal_code: e.target.value })} className="w-full border border-[#E0E0E0] rounded-lg px-3 py-2 text-sm max-w-xs" />
                      </div>
                      <label className="flex items-center gap-2 text-sm text-[#212121] cursor-pointer">
                        <input type="checkbox" checked={addressForm.is_default} onChange={(e) => updateAddressForm({ is_default: e.target.checked })} className="rounded border-[#E0E0E0] accent-[#1565C0]" />
                        Đặt làm địa chỉ mặc định
                      </label>
                    </div>
                    <div className="flex gap-2 justify-end px-5 py-4 border-t border-[#E0E0E0] bg-[#F5F6FA]/50 rounded-b-2xl">
                      <button type="button" onClick={closeAddressModal} disabled={addressSaving} className="px-4 py-2 text-sm border border-[#E0E0E0] rounded-lg hover:bg-white disabled:opacity-50">
                        Hủy
                      </button>
                      <button type="button" onClick={handleSaveAddress} disabled={addressSaving} className="px-4 py-2 text-sm bg-[#1565C0] text-white rounded-lg hover:bg-[#0D47A1] disabled:opacity-50">
